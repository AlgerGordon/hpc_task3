...
public:
    ...
    Process(int M_x_dots_total /*cols*/, int N_y_dots_total /*rows*/, double eps_in) {
        ...
        exchange_requests.resize(2*num_procs, MPI_REQUEST_NULL);
        exchange_statuses.resize(2*num_procs);
...
//        if ((num_procs < 4 && my_rank_world == 0) || (num_procs > 3 && my_rank_world == 3)) {
    ...
    void PrintW() {
        for (int row = 1; row < W_row_col.n_rows-1; ++row) {
            for (int col = 1; col < W_row_col.n_cols-1; ++col) {
                printf("w(x= %.10f ,y= %.10f )= %.10f\n",
                       A1 + (W_row_col.global_col_offset_11+col-1)*hx,
                       B1 + (W_row_col.global_row_offset_11+row-1)*hy,
                       W_row_col.el(row, col)
                );
            }
        }
    }
...
private:
    ...
    std::vector<MPI_Request> exchange_requests;
    std::vector<MPI_Status> exchange_statuses;
...
//                    printf("Number of threads = %d in CalculateR()\n", nthreads);
//                }
//            }
            int col = 0;
            for (col = 0; col < 2; ++col) {
    ...
    void CalculateTauParts() {
        ...
        double Ar_r_prod_sum = 0;
        double Ar_square_sum = 0;
...
//                {
//                    nthreads = omp_get_num_threads();
//                    printf("Number of threads = %d in CalculateTauParts()\n", nthreads);
//                }
//            }
            for (int col = 1; col < nCols-1; ++col) {
                ...
                ro_ij = (col == nCols - 2 || col == 1 ? 0.5 : 1) * (row == nRows - 2 || row == 1 ? 0.5 : 1);
                Ar_r_prod_sum += hx * hy * Ar_row_col.el(row, col)  * r_row_col.el(row, col) * ro_ij;
                Ar_square_sum += hx * hy * Ar_row_col.el(row, col)  * Ar_row_col.el(row, col) * ro_ij;
                r_square_sum += hx * hy * r_row_col.el(row, col) * r_row_col.el(row, col) * ro_ij;
            }
        }


        norms_local[Ar_r_prod] = Ar_r_prod_sum;
        norms_local[Ar_square] = Ar_square_sum;
        norms_local[r_square] = r_square_sum;
    }
...
//            }
            ...
            // Calculate Ar and tau
            // CalculateTauParts();

            #pragma omp for reduction(+:Ar_r_prod_sum,Ar_square_sum,r_square_sum)
            for (int row = 1; row < nRows-1; ++row) {
                double ro_ij = 0;
...
//            printf("Number of threads = %d in exchData\n", nthreads);
//        }
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (hn[(1-dir)*2] && hn[(1-dir)*2+1]) {
                ...
                MPI_Isend(shadow_bufs_paires_ptrs[i*2+SEND], 1-dir ? nCols : nRows, MPI_DOUBLE,
                        rank_send, 27, cart_comm, &exchange_requests[my_rank_world]);
            } else if (hn[(1-dir)*2 + std::abs(std::min(0,disp))]) {
                // std::cout << my_rank_world << " recv from " << rank_recv << std::endl;
                MPI_Irecv(shadow_bufs_paires_ptrs[i*2+RECV], 1-dir ? nCols : nRows, MPI_DOUBLE, rank_recv,
                         27, cart_comm, &exchange_requests[my_rank_world+1]);
            }
            // MPI_Barrier(MPI_COMM_WORLD);
        }
        MPI_Waitall(2*num_procs, exchange_requests.data(),exchange_statuses.data());
        // std::cout << "Left data exchange!" << std::endl;
...
//                {
//                    nthreads = omp_get_num_threads();
//                    printf("Number of threads = %d in MakeStep()\n", nthreads);
//                }
//            }
            for (int col = 0; col < nCols; ++col) {
